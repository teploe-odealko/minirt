...
/*   Updated: 2020/11/21 16:26:12 by bashleig         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			parse_coords(char **line, t_coords *coords)
{
	...
	if (!ft_isdigit(**line) && (**line != '-' && !ft_isdigit(line[0][1])))
		return (8);
	coords->x = moving_atoi(line);
	if (**line == ',')
		(*line)++;
	if (!ft_isdigit(**line) && (**line != '-' && !ft_isdigit(line[0][1])))
		return (8);
	coords->y = moving_atoi(line);
	if (**line == ',')
		(*line)++;
	if (!ft_isdigit(**line) && (**line != '-' && !ft_isdigit(line[0][1])))
		return (8);
	coords->z = moving_atoi(line);
	return (0);
}
...
int			parse_norm_vector(char **line, t_vector3 *vector3)
{
	...
	if (!ft_isdigit(**line))
		return (5);
	color->r = moving_atof(line);
	if (**line == ',')
		(*line)++;
	if (!ft_isdigit(**line))
		return (5);
	color->g = moving_atof(line);
	if (**line == ',')
		(*line)++;
	if (!ft_isdigit(**line))
		return (5);
	color->b = moving_atof(line);
	if (is_digit_in_bounds(color->r, 0, 255)
	&& is_digit_in_bounds(color->g, 0, 255)
	&& is_digit_in_bounds(color->b, 0, 255))
		return (0);
